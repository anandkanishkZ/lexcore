...
export default async function CasesPanel({ clientId }: { clientId: string }) {
    const result = await fetchCasesAction(1, 100, undefined, undefined, clientId);
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            {cases.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-700">No cases yet</p>
                    <p className="text-xs text-slate-500 mt-1">Cases opened for this client will show up here.</p>
                </div>
            ) : (
